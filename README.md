## Dashboard Creation
This is a dashboard creation pipeline made for Cell Morphomics Data.
The goal is to highlight the distributions of morphological feature between
control, LRRK2, PARKN and SNCA to understand the progression of disease on a single cell level. Moreover, the heterogeneity within a variant can then be studied by looking at Individual Cell lines.

## Summarised Findings
Please [V2_Rushik.pdf](https://github.com/Rushik-Rsenal/NeuRA/blob/main/V2_Rushik.pdf) for summarised findings. 


### Pipeline Steps
    1. Determine and Set features.
    2. Get Data
    3. Clean Data
    4. Generate Large Histogram.
    5. Generate Mini Histograms.
    6. Generate Box plots.
    7. Finalise the summary table.
    8. Save Dashboard as PNG.


